...
def _validate_backup_file(backup_path, live_db_path):
    """Open backup as SQLite and compare record counts. Returns (status, error_message)."""
    try:
...
def run_backup(backup_type='automated', user_id=None):
    ...
    with current_app.app_context():
        ...
        try:
            ...
            db.session.add(log)
            db.session.commit()
            print(f"[Backup] {backup_type} backup {status}: {backup_path}")
            return log
        except Exception as e:
            log = BackupLog(
                backup_type=backup_type,
                file_path=backup_path if 'backup_path' in dir() else 'unknown',
...
def panel():
    ...
    # Schema drift check result (stored in session for display)
...
def create_backup():
    ...
    # Record in audit log
    audit = AuditLog(admin_id=session['user_id'], action_type='BACKUP_TRIGGERED',
...
def restore_backup(backup_id):
    ...
    try:
        # 2. Replace live DB with backup
        shutil.copy2(log.file_path, db_path)
        # 3. Post-restore validation
        status, err = _validate_backup_file(db_path, log.file_path)
        if status == 'pass':
            audit = AuditLog(admin_id=session['user_id'], action_type='RESTORE_COMPLETED',
                             affected_record=f'backup_id:{backup_id}', timestamp=datetime.utcnow())
            db.session.add(audit)
            db.session.commit()
            return jsonify({'success': True, 'message': f'System restored to backup from {log.created_at.strftime("%Y-%m-%d %H:%M:%S")} UTC.'})
        else:
            # Revert to safety snapshot
            shutil.copy2(safety_log.file_path, db_path)
            return jsonify({'error': f'Post-restore validation failed. System reverted to pre-restore state. Error: {err}'}), 500
    except Exception as e:
        # Attempt revert
        try:
            shutil.copy2(safety_log.file_path, db_path)
...
def drift_check():
    ...
    try:
        ...
        conn.close()
        audit = AuditLog(admin_id=session['user_id'], action_type='DRIFT_CHECK',
                         affected_record=f'issues:{len(issues)}', timestamp=datetime.utcnow())
        db.session.add(audit)
        db.session.commit()
        if issues:
